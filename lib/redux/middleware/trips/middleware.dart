...
Future<TripsData> fetchTrips(Store<AppState> store) async {
  ...
  try {
    final response = await http.get(
        'http://localhost:3002/api/trips/all?owner_id=${store.state.currentUser.uid}',
    ...
    if (response.statusCode == 200) {
      ...
      if (results.error == null) {
        ...
        store.dispatch(new GetTripsAction(
          results.trips,
        ));
        store.dispatch(new ErrorAction(null, 'trips'));
        store.dispatch(new OfflineAction(
          false,
        ));
      } else if (results.error != null) {
        store.dispatch(new ErrorAction(results.error, null));
      }
    ...
    if (cacheData != null) {
      ...
      store.dispatch(new ErrorAction("Server is down", "trips"));
      store.dispatch(new SetTripsLoadingAction(false));
...
Future<DeleteTripData> deleteTrip(Store<AppState> store, String tripId) async {
  try {
    ...
    if (response.statusCode == 200) {
      ...
      store.dispatch(new DeleteTripAction(tripId, results.success));
      return results;
    } else {
      // If that response was not OK, throw an error.
      store.dispatch(new DeleteTripAction(tripId, false));
      store.dispatch(new SetTripsLoadingAction(false));
...
Future<CreateTripData> postCreateTrip(Store<AppState> store, dynamic data,
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (index != null && undo == true) {
        store.dispatch(new UndoTripDeleteAction(trip, index, true));
      } else {
        store.dispatch(new CreateTripAction(trip, true));
      }
