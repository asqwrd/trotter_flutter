...
class VaccineList extends StatelessWidget {
  ...
  buildRow(BuildContext context) {
    ...
    if (this.vaccines['recommended'] != null) {
      this.vaccines['recommeded'] = new List.from(this.vaccines['recommended'])
        ..addAll(recommended);
    } else {
    ...
    return Row(
      ...
      children: <Widget>[
        buildBody(context, this.vaccines['recommended'],
            'Required vaccinations', 'images/vaccines.png'),
        _showRequired
            ? buildBody(context, this.vaccines['required'],
                'Recommended vaccinations', 'images/vaccines.png')
            : Container(),
        _showRisk
            ? buildBody(context, this.vaccines['risk'], 'Viral risks',
                'images/risk.png')
            : Container(),
      ],
    );
